      ...
      <table class="table table-bordered table-responsive table-striped">
        ...
        <tr v-for="(item,index) in arrayData" v-bind:key="item.name">
          ...
          <td style='text-align: center'>
            ...
            <el-button type="primary" @click="edit(item.id)" style="margin-left: 10px" plain> 编辑</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      edit (id) {
        ...
        this.$router.push({path: '/web/edit?id=' + id})
      },
      editRelation (tagId) {
        // this.$router.push('/org/edit?orgid=' + agentId)
    ...
    created () {
      // var userid = localStorage.getItem('userid')
      api.request('get', 'web/list?userid=1&pageSize=20&pageIndex=0&webtype=help')
        .then(response => {
          console.log(response.data)
          this.totalCount = response.data.body.status.totalCount
          this.arrayData = response.data.body.webs
